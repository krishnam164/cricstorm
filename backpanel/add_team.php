...
// 1. FETCH TOURNAMENTS FOR SELECTION
...
if (isset($_POST['add_team'])) {
    ...
    // 2. GET TOURNAMENT NAME FOR FOLDER PATH
    ...
    $logo_name = $file_slug . "_logo_" . time() . ".png"; // Added time to avoid cache issues
    ...
    if (move_uploaded_file($_FILES["team_logo"]["tmp_name"], $target_dir . $logo_name)) {
        // Updated Table Name to 'team_master' as per your SQL
...
<div class="mb-10">
    <h2 class="text-2xl font-black text-slate-900 italic">Register <span class="text-teal-500">New Team</span></h2>
    <p class="text-xs text-slate-400 mt-1 uppercase tracking-widest font-bold">Assigning a squad to a specific league</p>
</div>
    ...
    <form method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-10 rounded-[3rem] border border-teal-50 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1">
                ...
                <select name="tournament_id" required class="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-teal-500 appearance-none">
            ...
            <div class="md:col-span-1">
                ...
                <input type="text" name="name" required placeholder="e.g. Chennai Super Kings" class="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold text-slate-700 outline-none focus:bg-white border border-transparent focus:border-teal-500">
            </div>
            ...
            <div class="md:col-span-1">
                ...
                <input type="text" name="owner_name" required class="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold text-slate-700 outline-none">
            </div>

            <div class="md:col-span-1">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 mb-2 block">Owner Mobile</label>
                <input type="text" name="mobile_no" required class="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold text-slate-700 outline-none">
            </div>
            ...
            <div class="md:col-span-1">
                ...
                <input type="number" name="total_points" id="t_points" required class="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold text-slate-700 outline-none" oninput="document.getElementById('r_points').value = this.value">
            </div>

            <div class="md:col-span-1">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 mb-2 block">Remaining Points</label>
                <input type="number" name="remaining_points" id="r_points" required class="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold text-slate-700 outline-none">
            </div>

            <div class="md:col-span-1">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 mb-2 block">Team Logo</label>
                <div class="relative h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center overflow-hidden">
                    ...
                    <label for="logo-in" class="cursor-pointer text-center w-full h-full flex flex-col items-center justify-center">
                        ...
                        <i id="logo-icon" class="fas fa-shield-alt text-slate-300 text-2xl mb-2"></i>
                        <p id="logo-text" class="text-[9px] font-black text-slate-400 uppercase">Upload Logo</p>
                    </label>
                </div>
            </div>
        </div> <div class="flex items-center gap-6">
            <button type="submit" name="add_team" class="bg-slate-900 text-white px-12 py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-widest hover:bg-teal-500 transition-all shadow-xl">
                Initialize Team
            </button>
            <a href="all_teams.php" class="text-[10px] font-black text-slate-400 uppercase">Cancel</a>
        </div>
    </form>
</div>
...
function preview(input, id) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            // Hide the icon and text when image is loaded
