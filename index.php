...
<header class="relative py-24 px-6 text-center bg-gradient-to-b from-teal-50/50 to-primary overflow-hidden">
    ...
    <div class="max-w-4xl mx-auto">
        <h1 class="text-5xl md:text-7xl font-extrabold text-slate900 mb-8 leading-[1.1] animate__animated animate__fadeInDown">
...
<section class="max-w-7xl mx-auto px-6 py-10">
    <div class="relative rounded-[3rem] overflow-hidden bg-slate-900 h-[600px] flex items-center shadow-2xl group animate__animated animate__zoomIn">
        ...
        <div class="relative z-10 px-12 md:px-20 max-w-2xl">
            ...
            <div class="flex flex-wrap gap-4 mt-8">
                <div class="flex items-center space-x-3 bg-white/10 backdrop-blur-md border border-white/10 px-5 py-3 rounded-2xl hover:bg-white/20 transition-all cursor-default">
                    <i class="fas fa-gavel text-brand"></i>
                    <span class="text-white text-sm font-bold tracking-tight">Hammer Logic</span>
                </div>
                <div class="flex items-center space-x-3 bg-white/10 backdrop-blur-md border border-white/10 px-5 py-3 rounded-2xl hover:bg-white/20 transition-all cursor-default">
                    <i class="fas fa-bolt text-brand"></i>
                    <span class="text-white text-sm font-bold tracking-tight">Instant Sync</span>
                </div>
            </div>
        </div>
    </div>
...
<section class="max-w-7xl mx-auto px-6 py-20 bg-primary">
    <div class="text-center mb-16">
        <h2 class="flex items-center justify-center gap-3 text-3xl md:text-4xl font-black text-slate-900">
            <span class="text-2xl">🏏</span> Advanced Features <span class="text-2xl">🏏</span>
        </h2>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <div class="bg-white p-10 rounded-2xl shadow-lg border border-slate-50 flex flex-col items-center text-center group transition-all hover:-translate-y-4 duration-300">
            <div class="w-20 h-20 bg-brand/10 text-brand rounded-2xl flex items-center justify-center text-4xl mb-6 group-hover:bg-brand group-hover:text-white group-hover:rotate-12 transition-all">
                <i class="fab fa-youtube"></i>
            </div>
            <h3 class="font-bold text-slate-900 text-lg">Live Streaming</h3>
        </div>
        ...
        <div class="bg-white p-10 rounded-2xl shadow-lg border border-slate-50 flex flex-col items-center text-center group transition-all hover:-translate-y-4 duration-300">
            ...
            <h3 class="font-bold text-slate-900 text-lg">Team Owner View</h3>
        </div>
        ...
        <div class="bg-white p-10 rounded-2xl shadow-lg border border-slate-50 flex flex-col items-center text-center group transition-all hover:-translate-y-4 duration-300">
            ...
            <h3 class="font-bold text-slate-900 text-lg">Remotely Bid</h3>
        </div>
        ...
        <div class="bg-white p-10 rounded-2xl shadow-lg border border-slate-50 flex flex-col items-center text-center group transition-all hover:-translate-y-4 duration-300">
            ...
            <h3 class="font-bold text-slate-900 text-lg">Customer support</h3>
        </div>
    </div>
</section>

<section id="working" class="max-w-7xl mx-auto px-6 py-24 bg-white rounded-[3rem] my-10 shadow-sm border border-teal-50">
    ...
    <div class="flex flex-col lg:flex-row items-center justify-between gap-16">
        <div class="lg:w-1/2 relative group">
            ...
            <img src="images/icon.png" 
                 alt="App Mockup" 
                 class="relative z-10 w-full max-w-md mx-auto drop-shadow-2xl transform -rotate-2 group-hover:rotate-0 group-hover:scale-110 transition-all duration-700 animate-bounce-slow">
        </div>
        ...
        <div class="lg:w-1/2 space-y-10">
            ...
            <div class="flex items-start gap-6 group">
                <div class="flex-shrink-0 w-12 h-12 bg-gradient-to-br <?php echo $step[3]; ?> rounded-full flex items-center justify-center text-white font-black shadow-lg group-hover:scale-110 group-hover:rotate-12 transition-all">
                    <?php echo $step[0]; ?>
                </div>
                <div>
                    <h4 class="text-xl font-bold text-slate-900 group-hover:text-brand transition-colors"><?php echo $step[1]; ?></h4>
                    <p class="text-slate-500 text-sm mt-1"><?php echo $step[2]; ?></p>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
...
<section class="max-w-7xl mx-auto px-6 py-16">
    <div class="flex items-center justify-between mb-12">
        <h2 class="text-3xl font-extrabold text-slate900 border-l-8 border-brand pl-5">Active Tournaments</h2>
        <span class="hidden md:block text-xs font-bold tracking-widest text-slate-400 uppercase animate-pulse">Live Marketplace</span>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        if(mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                // (Dynamic logic kept exactly the same as your code)
                $current_time = time();
            ...
            <div class="p-8">
                ...
                <div class="space-y-3 mb-8">
                    <div class="flex items-center text-slate-500 text-sm font-medium">
                        <i class="far fa-calendar-alt w-6 text-brand group-hover:scale-125 transition-transform"></i>
                        <span>Date: <?php echo date('d M, Y', $tournament_time); ?></span>
                    </div>
                    <div class="flex items-center text-slate-500 text-sm font-medium">
                        <i class="far fa-clock w-6 text-brand group-hover:scale-125 transition-transform"></i>
...
<style>
    /* Custom slow bounce for the icon image */
    @keyframes bounce-slow { 0%, 100% { transform: translateY(0) rotate(-2deg); } 50% { transform: translateY(-20px) rotate(2deg); } }
